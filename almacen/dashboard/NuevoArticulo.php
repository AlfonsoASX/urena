  ...
  <div class="container">
    <div class="row">
        <div class="col-lg-12 text-center text-info ">
          <h2 class="mt-5 font-weight-bold">Alta de Articulo Nuevo</h2>
        </div>
    </div>
      ...
      <div class="row ">
            ...
            <div class="col-lg-6 col-sm-10 text-center ">
                ...
                <form class="" action="NuevoArticulo.php" method="POST" enctype="multipart/form-data">
                  
                    <div class="form-inline pt-3 mr-5 float-right w-auto ">
                        <label for="articulo">Articulo:</label>
                        <input type="text" class="form-control" style="width: 300px;"  placeholder="" id="articulo" name="articulo" required pattern="[a-z0-9\_]+" title="Sin espacios ni caracteres especiales">
                    </div>
                    <div class="form-inline pt-3 mr-5 float-right ">
                        <label class="" for="marca">Marca:</label>
                        <input type="text" class="form-control" style="width: 300px;"  placeholder="" id="marca" name="marca" required pattern="[a-z0-9\_]+" title="Sin espacios ni caracteres especiales">
                    </div>
                    <div class="form-inline pt-3 mr-5 float-right">
                    <label>Foto:</label>
...
if ($_POST){
      ...
      $querya = $pdo->prepare("SELECT articulo, marca FROM articulos");
      $querya->execute();
      $codigos=0;
      while ($rowa = $querya->fetch(PDO::FETCH_ASSOC)) {
          if ($articulo==$rowa['articulo'] AND $marca==$rowa['marca']) {
            $codigos=1;
          }
      }
      if ($codigos==0) {  
        ...
        if ($size_imagen<=1000000){
          if($tipo_imagen == "image/jpg" OR $tipo_imagen == "image/jpeg" OR $tipo_imagen == "image/gif" OR $tipo_imagen == "image/png"){
              ...
              $pdo -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              $query = $pdo -> prepare("INSERT INTO articulos (articulo, marca, foto) 
                                      VALUES(:articulo, :marca, :foto) ");
              ...
              $query -> execute();
                ...
                ?>
                <script>alert("ERROR!!! EL ARTICULO YA EXISTE");</script>
                <?php
